...
case class Sensor(
    ...
    zone :           Int,
    sensorId :       Int,
    `type` :         Sensor.SensorType.Entry,
    usage :          Sensor.SensorUsage.Entry,
    numberOfBits :   Int,
    bconstant :      Int,
    thermistorSize : Int,
    tempCorrection : Int,
...
object Sensor extends SprayImplicits {
  ...
  /** Configurable trigger for an [[Action]]
    ...
    *
    */
  case class State(maxValue : Int, minValue : Int, `type` : StateType.Entry, actions : Seq[Action]) extends ReadonlyApiObject
  ...
  case class Action(`type` : ActionType.Entry, sendAlert : Boolean, sendUpdate : Boolean, activationDelay : Int,
      ...
      activateRelay : String, activateRelayOpen : Boolean) extends ReadonlyApiObject
